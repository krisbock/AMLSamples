...
class SvDkllTrainer: 
    ...
    def __init__(self,
       ...
       # Bernoulli likelood 
        ...
        self.nnet_layer = NeuralnetLayer(data_dim=X_train.size(1),
                                         output_dim=latent_dim).to(self.device)
        self.model=DKLModel(nnet_layer,
                           num_dim=latent_dim,
                           grid_bounds=grid_bounds,
                           grid_size=grid_size).to(self.device)
        self.hyper_params=hyper_params
        
       # Stochastic variational optimzer 
        self.optimizer=Adam([
            {'params': self.model.nnet_layer.parameters(),'lr':self.hyper.params.nn_lr, 'betas':(0.9, 0.999)},
    ...
    def train_svdkl(self,
        ...
        scheduler = MultiStepLR(self.optimizer,gamma=0.1,milestones=[0.5 * n_epochs, 0.75 * n_epochs])
        ...
        for epoch in range(1, self.hyper_params.epochs + 1):
            ...
            for batch_idx, (data, target) in enumerate(dataloader):
                ...
                if (batch_idx + 1) % 2 == 0:
                    ...
                    self.aml_run.log(loss.item()) if aml_run is not None
